...
namespace windowsthreadpool
{
	namespace internal
	{
		...
		public:
			...
			//
			SimpleCallBack(const Function Func, PVOID st, PTP_CALLBACK_ENVIRON pEnv,PVOID st2=NULL) : m_Func(Func), state(st), state2(st2)
			{			
	...
	class SimpleThreadPool 
	{
		private:
			...
			//
			template <class Function>
			bool QueueUserWorkItemInternal(Function cb, PVOID State, PVOID State2)
			{
				InitializeInfra(); 						
				windowsthreadpool::internal::SimpleCallBack<Function> *scb = new windowsthreadpool::internal::SimpleCallBack<Function>(cb, State, &CallbackEnvironment, State2);		
				return true;
			}
		...
		public:
			...
			template <class Function> 
			bool QueueUserWorkItem(Function cb, PVOID State, PVOID State2)
			{
				return QueueUserWorkItemInternal(cb, State, State2);
			}
