...
namespace windowsthreadpool
{
	...
	namespace internal
	{
		...
		public:
			WorkCallBack(const Function Func, PVOID st, PTP_CALLBACK_ENVIRON pEnv, PVOID st1=NULL) : m_Func(Func), state(st), state1(st1)
			{			
	...
	private:
		...
		template <class Function>
		bool QueueUserWorkItemInternal(Function cb, PVOID State, PVOID State1)
		{
			InitializeInfra(); 

			// Queues the work item to the default process-wide ThreadPool 			
			windowsthreadpool::internal::WorkCallBack<Function> *wcb = new windowsthreadpool::internal::WorkCallBack<Function>(cb, State, &CallbackEnvironment, State1);
	...
	public:
		...
		template <class Function> 
		bool QueueUserWorkItem(Function cb, PVOID State, PVOID State1)
		{
			return QueueUserWorkItemInternal(cb, State,State1);
		}
