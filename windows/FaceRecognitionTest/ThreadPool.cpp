...
CThreadPool& CThreadPool::GetThreadPool(){
...
CThreadPool::CThreadPool(int nPoolSize, bool bCreateNow, int nWaitTimeForThreadsToComplete)
{
	...
	m_taskCompletedHandle=CreateEvent(NULL, TRUE, FALSE, NULL);
...
CThreadPool::~CThreadPool()
{
	...
	CloseHandle(m_taskCompletedHandle);
}
//------------------------------------------------------------------------------
...
void CThreadPool::FinishNotify(DWORD dwThreadId)
{
	...
	{	
		...
		else
		{
			...
			//std::cout<<"in Finish() "<<m_functionList.size()<<"\n";
			...
			if(GetWorkingThreadCount()==0){
				//std::cerr<<"empty\n";
				SetEvent(m_taskCompletedHandle);
			}
		}
	}	
}
...
/* Parameters	: pFunc - function pointer of type ThreadProc
  ...
  *****************************************************************************/
bool CThreadPool::Run(LPTHREAD_START_ROUTINE pFunc, LPVOID pData, LPVOID pDataAux,
					  ThreadPriority priority)
{
	...
	for(iter = m_threads.begin(); iter != m_threads.end(); iter++)
	{
		...
		if(ThreadData.bFree)
		{
			...
			if(GetWorkingThreadCount()==0)
				ResetEvent(m_taskCompletedHandle);
...
bool CThreadPool::WaitForAllTasksCompletion()
{
	bool isOk=true;
	if(WaitForSingleObject(m_taskCompletedHandle,10000)==WAIT_TIMEOUT){
		isOk=false;
	}
	return isOk;
}