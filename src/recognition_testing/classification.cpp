...
#include <sstream>
template<typename T> string get_name(string prefix, T param){
    ostringstream os;
    os << prefix << ", " << param;
    return os.str();
}
...
public:
    KNNClassifier(int k):Classifier(get_name("k-NN",k)),K(k) {}
    void train(){}
...
public:
    PNNwithClusteringClassifier(int no_clusters):Classifier(get_name("PNN with clustering",no_clusters)),num_clusters(no_clusters) {}
    void train();
    int predict(const Feature_vector& inputFeatures);
private:
...
//=====================================================
class PNNFSClassifier: public PNNClassifier{
public:
    PNNFSClassifier(double scale=1.0,bool bf=true):PNNClassifier(bf,get_name("OS-PNN",scale)),features_scale(scale){}
    void train();
protected:
...
private:
    ...
    static const float output_delta;
};

const float PNNFSClassifier::output_delta=fastlog(0.9f);

FEATURE_TYPE PNNFSClassifier::normalize(const Feature_vector& feature_vector, size_t fi){
#if 0
...
int PNNFSClassifier::predict_bf(const Feature_vector& inputFeatures){
    vector<float> outputs(num_of_classes,0);
    vector<double> cos_vals(J),sin_vals(J);
    ...
    for(size_t fi=0;fi<max_fi;++fi){
        ...
        for(size_t i=0;i<num_of_classes;++i){
            ...
            /*if(abs(i-tmp_dataset[test_ind].output)<=2 && fi<=8){
                qDebug()<<"J="<<J<<" real class="<<tmp_dataset[test_ind].output<<" class="<<i<<" fi="<<fi<<" probab="<<probab<<" val="<<val<<" acos (val)="<< acos(cos_vals[0])/PI<<" cos_model"<<a[model_ind+1]<<" model="<<acos(a[model_ind+1])/PI;
            }*/
            //outputs[i] += weights[fi]*fastlog(probab);//num_of_cont_features;
...
int PNNFSClassifier::predict_sequentional(const Feature_vector& inputFeatures){
    int bestClass=-1;
...
void testClassification(){
    ...
    classifiers.push_back(new PNNFSClassifier(1.0,true));
    classifiers.push_back(new PNNFSClassifier(0.33,true));
#if NO_PCA_FEATURES>32
    classifiers.push_back(new PNNClassifier(false));
    classifiers.push_back(new PNNFSClassifier(1.0,false));
    classifiers.push_back(new PNNFSClassifier(0.33,false));
#endif
    ...
    //for(double fraction=3;fraction<=20;fraction+=2){
        ...
        for(size_t shuffle_num=0;shuffle_num<1;++shuffle_num){
            for(size_t test_num=0;test_num<num_of_tests;++test_num){
                ...
                for(size_t fi=0;fi<num_of_cont_features;++fi){
                    ...
                    //cout<<"before PCA:"<<fi<<' '<<avgValues[fi]<<' '<<count<<' '<<(avgValues[fi]/count);
